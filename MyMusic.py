...
def display_sheet_card(file, metadata):
    ...
    # Preview link (underlined and same color as default text)
    with col1:
        if metadata.get("preview_link"):  # Use the preview_link from metadata
            st.markdown(
                f"""
                <div style="text-align: center;">
                    <a href="{metadata.get('preview_link')}" target="_blank" style="text-decoration: underline; color: inherit; font-weight: bold; font-size: 1rem;">Preview</a>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                """
                <div style="text-align: center;">
                    <p style="font-size: 1rem; color: gray;">No preview available</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
